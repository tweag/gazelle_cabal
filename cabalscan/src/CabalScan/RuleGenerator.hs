...
generateRulesForCabalFile cabalFilePath = do
  ...
  let library = Cabal.condLibrary pd
      ...
      pkgDesc = Cabal.packageDescription pd

  libRules <-
    traverse (generateRule LIB cabalFilePath pkgDesc) libraries
  executablesRules <-
    traverse (generateRule EXE cabalFilePath pkgDesc) executables
  testSuiteRules <-
    traverse (generateRule TEST cabalFilePath pkgDesc) testsuites
  benchmarkRules <-
    traverse (generateRule BENCH cabalFilePath pkgDesc) benchmarks
...
-- | Check if a module is the autogenerated Paths_ module for the given package
-- Uses Cabal's official autogenPathsModuleName function
isPathsModule :: Cabal.PackageDescription -> FilePath -> Bool
isPathsModule pkgDesc modulePath = 
  Cabal.toFilePath (Cabal.autogenPathsModuleName pkgDesc) == modulePath

getSources :: Cabal.PackageDescription -> Cabal.BuildInfo -> FilePath -> String -> [FilePath] -> IO [FilePath]
getSources pkgDesc bi cabalFilePath attrName someModules = do
    let otherModules = filter (not . isPathsModule pkgDesc) $ map Cabal.toFilePath (Cabal.otherModules bi)
...
generateRule
  ...
  -> IO (Maybe RuleInfo)
generateRule compType cabalFilePath pkgDesc (CondNode var deps branches) =
 ...
 where
    ...
    dataFiles = Cabal.dataFiles pkgDesc
    ...
    convert compVar = do
      ...
      srcs <- getSources pkgDesc bi cabalFilePath attrName mods
      return $ RuleData
        {
